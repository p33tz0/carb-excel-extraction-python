...
df = pd.read_excel(r'.\hhtiedot\makeiset.xlsx')
print(df.columns)
...
for i in range(len(df)):
    ...
    formatted_string = f"Food(\n" \
                       ...
                       f"    category: Category(\n" \
                       f"        name: 'Makeiset, naposteltavat',\n" \
                        f"       image: 'assets/images/candy.png')),\n" \
                       f")"
...
with open('.\outputs\outputmakeiset.txt', 'w', encoding='utf-8') as file:
    file.writelines('\n'.join(formatted_data))